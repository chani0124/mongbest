...
def best(items):
    ...
    return sorted(items, key=lambda x: x["total"])[0]

# ---- 각 판매처 크롤러(실패해도 전체 진행) ----
def crawl_coupang(page):
    ...
    try:
        ...
        for li in soup.select("ul.search-product-list li.search-product"):
            ...
            price = to_int_krw(price_el.get_text("", strip=True) if price_el else "")
            items.append({"name": name, "price": price, "url": href, "delivery": "로켓/일반", "shipping": 0})
        b = best(items)
...
def crawl_11st(page):
    ...
    try:
        ...
        for p in soup.select("div.product_list div.c_product_top"):
            ...
            price = to_int_krw(price_el.get_text("", strip=True) if price_el else "")
            items.append({"name":name,"price":price,"url":href,"delivery":"일반","shipping":0})
        b = best(items)
...
def crawl_gmarket(page):
    ...
    try:
        ...
        for c in soup.select("div.box__component-itemcard"):
            ...
            price = to_int_krw(price_el.get_text("", strip=True) if price_el else "")
            items.append({"name":name,"price":price,"url":href,"delivery":"일반","shipping":3000})
        b = best(items)
...
def crawl_ssg(page):
    ...
    try:
        ...
        for it in soup.select("div.csrch_lst li"):
            ...
            price = to_int_krw(price_el.get_text("", strip=True) if price_el else "")
            items.append({"name":name,"price":price,"url":href,"delivery":"쓱배송","shipping":0})
        b = best(items)
...
def crawl_naver(page):
    ...
    try:
        ...
        for a in soup.select("a"):
            ...
            items.append({"name":title[:150],"price":price,"url":href,"delivery":"스토어별","shipping":2500})
        b = best(items)
...
def run_all():
    ...
    # ⚠️ 문자열 합치기로 안전하게 JS 파일 생성 (삼중따옴표 X)
    js_obj = {
        "product": "몽베스트 생수 2L x 12",
        "lastUpdated": now,
        "sellers": sellers
    }
    content = "/* Auto-generated by scripts/scrape.py */\n" \
              "window.MB_DATA = " + json.dumps(js_obj, ensure_ascii=False) + ";\n"
